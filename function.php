...
function upload()
{
    ...
    if ($error === 4) {
        echo "<script>alert('Choose a picture first!');</script>";
        return false;
    }
    ...
    if (!in_array($ext, $extValid)) {
        echo "<script>alert('What you uploaded is not a picture!');</script>";
        return false;
    }

    // Jika ukuran gambar lebih dari 3.000.000 byte maka akan menampilkan alert dibawah
    if ($ukuranFile > 3000000) {
        echo "<script>alert('Your image size is too big!');</script>";
        return false;
    }
